        ...
        private void dgv_productos_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (iRow >= 0 && iColumn >= 0)
            {
                producto = new Producto()
                {
                    ...
                    material = dgv_productos.Rows[iRow].Cells["MATERIAL"].Value.ToString(),
                    kilates = dgv_productos.Rows[iRow].Cells["GEMA"].Value.ToString(),
                    //kilates = dgv_productos.Rows[iRow].Cells["EMAIL"].Value.ToString(),
                    stock = Convert.ToInt32(dgv_productos.Rows[iRow].Cells["STOCK"].Value),
                    precio = Convert.ToDecimal(dgv_productos.Rows[iRow].Cells["PRECIO VENTA"].Value)
